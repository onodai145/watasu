...
- **OIDC 認証** — Google・Keycloak・Auth0 など任意の OIDC プロバイダに対応。初回ログイン時に自動でユーザーを作成（JIT プロビジョニング）
...
**ユーザープロビジョニング**  
初回ログイン時に自動でユーザーレコードを作成します。DB にユーザーが0人の状態での最初の OIDC ログインは `admin` ロールで作成されます。2回目以降のログイン時は表示名・メールアドレスを IdP の最新情報に同期します。ロールの変更は管理画面から行えます。
...
- OIDC ユーザーはアプリ側でのプロフィール編集（パスワード・TOTP）に対応していません。表示名・メールアドレスはログインのたびに IdP から自動同期されます
